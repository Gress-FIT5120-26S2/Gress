...
`GET /api/food-presets/suggestion` reuses an enabled preset by canonical name or exact alias. On a miss, the App can explicitly call `POST /api/food-presets/generate`: Gemini returns validated structured guidance, Cloudflare FLUX creates one icon, Sharp normalises it to a transparent 256×256 PNG, and Express caches both in Supabase. The endpoint has a process-local limit of five new foods per device per hour; production deployments should add an equivalent gateway-level limit across instances.
