...
`GET /api/health` confirms that Express can authenticate to Supabase. All `/api` requests first pass a generous database-backed IP perimeter limit shared by Vercel instances. After device authentication, ordinary business traffic uses an independent per-device fixed-window limit so devices sharing one public IP do not consume each other's allowance; recovery, invite joining, photo recognition, and AI generation retain tighter limits and return `429` with `Retry-After`. Assistant model generation defaults to 30 messages per device per 15 minutes, while feedback and action confirmation use a separate 120-per-hour mutation allowance because they do not invoke the model. Browser origins are denied unless listed exactly in the comma-separated `CORS_ALLOWED_ORIGINS`; native app requests without an `Origin` header remain allowed. `POST /api/photo-recognition` accepts one JPEG, PNG, or WebP image in the multipart field `file`, checks the requesting `Device-ID`, and proxies the image to the recognition service without saving it to disk or Supabase. Keep all calls that use the secret key or private service configuration in this folder.
...
`GET /api/barcode-products/:barcode` validates the GTIN check digit, queries Open Food Facts v3.6 with an identified User-Agent, and returns only normalized product name, brand, package, category, storage guidance, and image fields. The response is an editable prefill and never supplies the physical item's expiry date or purchase price. A short-lived process cache reduces duplicate upstream reads; the authenticated route also has a device-level database rate limit.
