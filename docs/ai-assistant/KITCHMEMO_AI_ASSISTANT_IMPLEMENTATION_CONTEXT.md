...
The full-screen chat now avoids the software keyboard on both iOS and Android, keeps the conversation list flexed above the composer, and scrolls to the latest context after the keyboard finishes opening. The four larger quick prompts remain in the empty-conversation introduction; after the first turn they move into a compact horizontal strip above the input. The AI fallibility and confirmation notice is part of the scrollable conversation footer instead of occupying fixed composer space.
