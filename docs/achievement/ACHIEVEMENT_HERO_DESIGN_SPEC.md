...
山峰等级区域保留等级标题、升级文案、山峰名称与海拔、山峰和云层、V 形路线、相邻等级节点、三项数据。2026-09-23 的成就页概览改版缩短山峰区，下面依次展示单条下一步行动、环保成果摘要和两枚徽章预览；完整挑战、环保明细、最近 XP 与奖牌馆通过点击进入。等级和业务状态仍以同一成就快照为准。
...
概览版组件高度约为屏幕宽度的 0.96 倍，常见手机限制在 350–430 dp；字体放大时按标题实测高度扩展。山峰主体占组件宽度的 45%–58%。组件不自带状态栏、返回按钮、页面导航或下方内容卡片。天空渐变铺满可用宽度，山峰、路线和交互区域保持居中且不超过 560 dp。
