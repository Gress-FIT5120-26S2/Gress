...
> 状态：团队已确认；第二、第三阶段已在开发库完成 migration 与端到端验证（Confirmed / Phase 3 development verified）
> 版本：0.3
> 最后更新：2026-09-12
...
- `20260912010000_inventory_outcome_accounting.sql` 已在开发库登记并验证购买价格必填、临期使用、硬过期拒绝使用和丢弃结果流水。
- 本地 `20260912020000_achievement_dashboard.sql` 已定义五级山峰、8 项成就、冰箱 XP 账本、周期奖励与聚合 RPC；Express 和 Expo 已接入 `GET /api/achievements`。

### 3.2 当前尚未具备

- `20260912020000_achievement_dashboard.sql` 已在开发库应用并通过成就端到端验证；生产库尚未应用，因此依赖它的接口和 App 仍不得发布到生产。
- 超过 `use_by_at` 的剩余库存尚未由定时任务自动结算；当前只在用户移除和相关 mutation 边界执行安全校验。
- 误判结果后的补偿 XP 流水与面向用户的立即更正入口尚未实现。
- 五座山峰的正式授权素材、分层动效和等级切换视觉仍属于后续视觉阶段。
...
第一版把“浪费率降低”明确为跨单位可比较的结算批次口径：`丢弃结算批次数 / 全部结算批次数`。当前完整周和此前连续四个完整自然周均至少结算 5 个批次，且当前周相对四周合并基线至少下降 10 个百分点时，奖励 40 XP。每周使用包含周起始日期的稳定 `source_key`，重复对账不得重复奖励。
...
- [x] 新库存购买价格必填，第一版币种固定 AUD。
- [x] 历史空价格使用 `legacy_unknown`，不伪造金额。
- [x] 只有 `use_by` 是硬性禁止食用的日期。
- [x] 临期移除默认视为及时使用，但提供立即更正入口。
- [x] 硬过期剩余库存自动或安全对账为 `discard`。
- [x] XP 和成就属于共享冰箱，等级只升不降。
- [x] 第一版采用五个山峰等级及本文阈值。
- [x] 每周规则使用冰箱统一时区。
- [x] 第一版不实现需要手动领取的任务系统。
- [x] 所有 schema 和数据库行为变化使用新的 migration。
