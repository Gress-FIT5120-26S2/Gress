-- Arthur: NarIyirm
-- 中文：移除确认函数中的无用局部变量，并把补货数量上限补到数据库边界；上一份已部署 migration 保持不变。
-- EN: Remove an unused local and enforce restock quantity caps at the database boundary while preserving the previously deployed migration unchanged.
...
comment on function public.confirm_assistant_pending_action(text, uuid) is
  'Atomically validates and executes one creator-scoped assistant action with expiry, version, and unit-aware quantity protection.';
